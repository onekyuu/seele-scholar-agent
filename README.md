...
### 可选字段：ConsistencyChecker Map-Reduce 子检查 Prompts

以下六个字段默认为空字符串。留空时，节点自动使用内置默认模板；提供时，将覆盖默认模板，可完全定制各维度检查的指令风格和输出格式。
...
7. **ConsistencyChecker**：术语、逻辑、引用三个维度并行独立检查，每路只接收最小必要上下文，最后聚合结果
8. **ReferenceGenerator**：生成参考文献列表（CrossRef 补全元数据，自动回退）
